...
=== Configurazione del proxy con gli utenti di dominio

BlueXP non supporta gli utenti di dominio durante la configurazione di un server proxy per il connettore. Quando si inserisce un nome utente per il server proxy, questo deve essere un utente locale.
