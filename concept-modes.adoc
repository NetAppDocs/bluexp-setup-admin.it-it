...
| Backup e recovery  a| 
In modalità privata, il BlueXP backup and recovery supportano solo il backup e il ripristino dei dati del volume ONTAP . https://docs.netapp.com/us-en/bluexp-backup-recovery/prev-ontap-protect-journey.html#support-for-sites-with-limited-internet-connectivity["Visualizza l'elenco delle destinazioni di backup supportate per i dati dei volumi ONTAP"^]
