...
+
Si ricorda che questo criterio contiene solo le autorizzazioni necessarie per avviare l'istanza di Connector in AWS da BlueXP. link:reference-permissions-aws.html["Visualizza i permessi richiesti per l'istanza del connettore"].
...
Creare policy IAM in AWS e associarle a un ruolo IAM. Quando si crea il connettore da AWS Marketplace, viene richiesto di selezionare il ruolo IAM.

.Fasi
. Dalla console IAM, creare una policy:
+
.. Fare clic su *Policy > Create policy* (Criteri > Crea policy).
.. Selezionare *JSON* e copiare e incollare il contenuto di link:reference-permissions-aws.html["Criteri IAM per il connettore"].
...
. Nella console IAM, creare un ruolo IAM:
+
.. Fare clic su *ruoli > Crea ruolo*.
.. Selezionare *servizio AWS > EC2*.
.. Aggiungere le autorizzazioni allegando i criteri creati nel passaggio precedente.
...
Se si installa manualmente il software del connettore sul proprio host Linux in AWS, è possibile fornire le autorizzazioni nei seguenti modi:
