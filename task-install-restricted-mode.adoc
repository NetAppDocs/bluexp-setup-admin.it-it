...
.Prima di iniziare
La persona che configura BlueXP  Connector deve accedere a BlueXP  utilizzando un login che non appartiene a un'organizzazione BlueXP  che opera in modalità standard.

Se il tuo login BlueXP  è associato a un'organizzazione che opera in modalità standard, dovrai registrarti con un nuovo login BlueXP  o con le credenziali del sito di supporto NetApp che non appartengono a un'organizzazione. In caso contrario, non viene visualizzata l'opzione di attivazione della modalità limitata nella schermata di configurazione.
