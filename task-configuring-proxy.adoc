...
La configurazione del connettore per l'utilizzo di un server proxy consente l'accesso a Internet in uscita quando un indirizzo IP pubblico o un gateway NAT non è disponibile. Il server proxy fornisce connettività in uscita solo per il connettore, non per i sistemi Cloud Volumes ONTAP.

Se i sistemi Cloud Volumes ONTAP non dispongono di una connessione Internet in uscita per inviare messaggi AutoSupport, BlueXP configura automaticamente i sistemi Cloud Volumes ONTAP in modo che utilizzino un server proxy incluso nel connettore. L'unico requisito è garantire che il gruppo di sicurezza del connettore consenta le connessioni in entrata sulla porta 3128. Aprire questa porta dopo aver distribuito il connettore.
