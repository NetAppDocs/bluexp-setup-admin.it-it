...
== Comprendere il funzionamento della modalità limitata
...
=== Accesso a Internet in uscita per installazioni manuali
...
\https://occmclientinfragov.azurecr.us
Questo endpoint è richiesto solo nelle regioni governative di Azure. | Per aggiornare il connettore e i relativi componenti Docker. 
|===
...
In modalità limitata, l'interfaccia utente di BlueXP è accessibile dal connettore. Quando si utilizza l'interfaccia utente di BlueXP, si contatta alcuni endpoint per completare le attività di gestione dei dati. Il computer che esegue il browser Web deve disporre di connessioni ai seguenti endpoint.
...
.Fasi
. Se si prevede di installare manualmente il software sul proprio host, attivare un'identità gestita assegnata dal sistema su in modo da poter fornire le autorizzazioni Azure richieste attraverso un ruolo personalizzato.
+
